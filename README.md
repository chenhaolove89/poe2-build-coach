...
**这一页把策略和玩法接起来的地方是生态。** 树里有 88 个节点的效果自己写着「在 X 区域」(`[Biome|Mountain] Areas ...`),而地图表的每个地区都带生态 —— 于是「哪些异界节点帮到你正在刷的图」有数据可依,不需要谁去编一份映射。在地图页点任意地区的生态,就能跳到这一页按该生态筛选(三个城市地区会折成树自己用的 `City` 写法)。
...
数据由 `scripts/build-atlas.mjs` 从 `repoe-fork.github.io/poe2` 的 `passive_skill_trees/Atlas.json` 生成到 `packages/data/atlas.json`。该文件不带坐标 —— 节点位于所属 group 的 (x, y) 加上所在轨道的角度偏移 —— 所以构建脚本按游戏自己的约定把坐标烘进去(`sin` 定 x、`-cos` 定 y,即 0 号槽朝上、顺时针),渲染端只读坐标、不做三角函数。几何正确性由子树聚集度验证:6 棵机制子树各自紧凑成簇并环绕主树,与异界树应有的形态一致。
